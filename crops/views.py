...
def result(request):
   ...
   cls=joblib.load('D:\ProjectKrishak\ML_Model_Django\crops')
   lis=[]
   lis.append(12)
   lis.append(20)
   lis.append(50)
...
class dataApi(APIView):
   def get(self,request):
        ...
        print("8888888888888888888888888888888888888888")
        cls=joblib.load("D:\ProjectKrishak\ML_Model_Django\crops\model.pkl")
        lis=[]
   ...
   def post(self,request):
        print("*******************")
        Data.objects.create(n_value=request.data['n'],p_value=request.data['p']
                            ,k_value=request.data['k'],temp=request.data['t']
                            ,humidity=request.data['h'])
        data=Data.objects.all()
        seri=UserSerializer(data,many=True)
